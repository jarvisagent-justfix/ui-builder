...
// 4. PAGINE DAL CONFIG
// —————————————————————————————————————
const appConfig = window.__appConfig || { name: 'App', pages: {} };
...
function createPagesFromConfig() {
  ...
  if (names.length === 0) {
    pages.add({ id: 'page-1', name: 'Pagina 1', component: '<div style="padding:24px;font-family:Inter,sans-serif"><h2>Nuova App</h2><p>Trascina i blocchi qui</p></div>' });
    renderPageTabs();
    return;
  }
  var existing = pages.getAll();
  existing.forEach(function(p) { try { pages.remove(p); } catch(e) {} });
  names.forEach(function(name) {
    var content = config.pages[name];
